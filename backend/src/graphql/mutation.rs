...
impl RootMutation {
    ...
    async fn signup(
        ...
        #[graphql(validator(StringMinLength(length = "6")))] password: String,
    ) -> FieldResult<User> {
        let db = ctx.data_unchecked::<AppContext>().db_pool.clone();
        let collection = db.database("AppStore").collection("users");
        // check if email existed
        let existed = RootQuery.user_by_email(ctx, email.clone()).await;
        match existed {
            Err(_) => {
                // hash password
                ...
                RootQuery.user_by_id(ctx, new_user_id).await
            }
    ...
    async fn create_asset(
        ...
        let db = ctx.data_unchecked::<AppContext>().db_pool.clone();
        let collection = db.database("AppStore").collection("assets");
        ...
        RootQuery.asset_by_id(ctx, new_asset_id).await
    }
    ...
    async fn create_org(
        ...
        let db = ctx.data_unchecked::<AppContext>().db_pool.clone();
        let collection = db.database("AppStore").collection("orgs");
        ...
        RootQuery.org_by_id(ctx, new_org_id).await
    }
    ...
    async fn create_product(
        ...
        let db = ctx.data_unchecked::<AppContext>().db_pool.clone();
        let collection = db.database("AppStore").collection("products");
        ...
        RootQuery.product_by_id(ctx, new_product_id).await
    }
    ...
    async fn create_profile(
        ...
        let db = ctx.data_unchecked::<AppContext>().db_pool.clone();
        let collection = db.database("AppStore").collection("profiles");
        ...
        RootQuery.profile_by_id(ctx, new_profile_id).await
    }
    ...
    async fn update_profile(
        ...
        match collection
            ...
            .await
        {
            Ok(_updated_data) => RootQuery.profile_by_id(ctx, id).await,
            Err(_e) => Err(FieldError::from("Failed to changed password.")),
        }
    }
    // ========================== ROLE =============================
    async fn create_role(
        ...
        let db = ctx.data_unchecked::<AppContext>().db_pool.clone();
        let collection = db.database("AppStore").collection("roles");
        ...
        RootQuery.role_by_id(ctx, new_role_id).await
    }
